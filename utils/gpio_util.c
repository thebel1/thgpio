 ...
 */
int
gpio_readReg(uint64_t baseAddr,   // IN
             uint64_t reg,    // IN
             uint64_t *ptr)   // OUT
{
 ...
 */
int
gpio_writeReg(vmk_VA baseAddr,  // IN
              vmk_uint32 reg,   // IN
              vmk_uint32 val)   // IN
{
 ...
 */
int
gpio_funcSelPin(vmk_VA baseAddr,   // IN
                vmk_uint32 pin,    // IN
                vmk_uint32 func)   // IN
{
   VMK_ReturnStatus status = VMK_OK;
 ...
 */
int
gpio_setPin(vmk_VA baseAddr, // IN
            vmk_uint32 pin)  // IN
{
   VMK_ReturnStatus status = VMK_OK;
 ...
 */
int
gpio_clrPin(vmk_VA baseAddr, // IN
            vmk_uint32 pin)  // IN
{
   VMK_ReturnStatus status = VMK_OK;
 ...
 */
int
gpio_levPin(vmk_VA baseAddr,  // IN
            vmk_uint32 pin,   // IN
            vmk_uint32 *ptr)  // OUT
{
   VMK_ReturnStatus status = VMK_OK;
 ...
 */
int
gpio_setPull(vmk_VA baseAddr,   // IN
             vmk_uint32 pin,    // IN
             vmk_uint8 pud)     // IN
{
   int ret = 0;
   uint32_t reg, shift, oldPud, newPud;

   shift = (pin % 16) * 2;
   reg = gpioPinToPudReg[pin / 16];
   gpio_readReg(baseAddr, reg, &oldPud);
   ...
   return gpio_writeReg(baseAddr, reg, newPud);
}
