 ...
 * gpio_util.h --
 * 
 *    UW utility for controlling the GPIO device.
 */
...
VMK_INLINE VMK_ReturnStatus gpio_readReg(uint64_t baseAddr,
                                         uint32_t reg,
                                         uint32_t *ptr);

VMK_INLINE VMK_ReturnStatus gpio_writeReg(uint64_t baseAddr,
                                          uint32_t reg,
                                          uint32_t val);

VMK_INLINE VMK_ReturnStatus gpio_funcSelPin(uint64_t baseAddr,
                                            uint32_t pin,
                                            uint32_t func);

VMK_INLINE VMK_ReturnStatus gpio_setPin(uint64_t baseAddr,
                                        uint32_t pin);

VMK_INLINE VMK_ReturnStatus gpio_clrPin(uint64_t baseAddr,
                                        uint32_t pin);

VMK_INLINE VMK_ReturnStatus gpio_levPin(uint64_t baseAddr,
                                        uint32_t pin,
                                        uint32_t *ptr);

VMK_INLINE VMK_ReturnStatus gpio_setPull(uint64_t baseAddr,
                                         uint32_t pin,
                                         uint8_t pud);
