...
/* For chardev */
static thx_CharDev_t gpio_CharDev;
 ...
 */
static thx_CharDevCallbacks_t gpio_CharDevCBs = {
...
init_module(void)
{
   ...
   /*
    * Init GPIO driver & char dev driver
    */
   gpio_drvInit(&gpio_Driver, &gpio_Device);
   thx_charDevInit(gpio_Driver.moduleID,
                   gpio_Driver.heapID,
                   gpio_Driver.logger);
...
{
   VMK_ReturnStatus status = VMK_OK;
   thx_CharDevProps_t charDevProps;

   charDevProps.driverHandle = gpio_Driver.driverHandle;
   charDevProps.logicalBusType = gpio_logicalBusType;
   charDevProps.parentDevice = device;
   charDevProps.charDev = &gpio_CharDev;
   charDevProps.logicalPort = gpio_logicalPort;
   charDevProps.callbacks = &gpio_CharDevCBs;
   
   status = thx_charDevRegister(&charDevProps);
